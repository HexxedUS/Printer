...
module Logo(h=1, scale=1) union() {
  LogoBloom(h*7/8, scale);
  translate([2, 0, 0]) rotate([-5, 0, 0])
    LogoText(h*9/8, scale);
}

module LogoBloom(h=1, scale=1) scale([scale, scale, 1])
  for(vector=[
    [0, 0, 0],
...
module LogoText(h=1, scale=1, text="Hexxed") translate([0, 0, h/2]) scale([scale, scale, h]) {
